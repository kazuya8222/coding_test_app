...
export const StartInterview: React.FC<StartInterviewProps> = ({
  ...
  useEffect(() => {
    ...
    if (countdown === 0) {
      // Just navigate to the interview page, don't call the startInterview function here
  ...
  }, [countdown, interviewId, navigate]);
  ...
  const handleReadyClick = async () => {
    setIsLoading(true);
    try {
      // Call onStart only once here
      await onStart(interviewId);
      // Start the countdown after successfully calling onStart
      setCountdown(3);
    } catch (err) {
